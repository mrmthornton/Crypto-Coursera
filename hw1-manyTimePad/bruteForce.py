...
def nextByte(i,L,N):
    if i < L:
        for n in range(256):
            ...
            nextByte(i+1,L,N)
    if i == L:
        key = "".join([chr(n) for n in N])
        decrypt = [encrypt(key, msg) for msg in MSGS]
        #if decrypt[0].isalnum():
        if decrypt[0].find('T') == 0 and decrypt[0].isalnum():
            print "{}, {}, {}, {}".format(decrypt[0], decrypt[1], decrypt[2], decrypt[3])

if __name__ == '__main__':

    shortKey = [0,0,0,0]
    N = [0 for n in range(len(shortKey))]
    print "N = {}".format(N)
    assert str('\0') == chr(0)
    key = "".join([chr(n) for n in N])
    print key
