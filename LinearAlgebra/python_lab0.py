...
from python_lab import list_of_numbers, range_and_zip
coursera = 1
...
remainder_without_mod = 304811 - (304811//47)*47
print(remainder_without_mod)


## 3: (Task 3) Divisibility
divisible_by_3 = 673 + 909 - ((673 + 909)//3)*3 == 0
...
first_five_squares = {n*n for n in {1,2,3,4,5} }
...
X2 = { -1, 0, 1 }
Y2 = { -2, 111, 2 }
print({x*y for x in X2 for y in Y2})
...
S_intersect_T = {e for e in S if e in T}
print(S_intersect_T)
...
print(list_of_numbers, list_average)
...
## 13: (Task 13) Three-element tuples summing to zero / or minus one
S = {-4, -2, 1, 2, 5, 0}
S = [-4, -2, 1, 2, 5, 0]
...
#zero_sum_list = [ (a, b, c) for a in S for b in S if b!=a for c in S if c!=a and c!=b if a+b+c==-1]
print(zero_sum_list)
...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [ (a, b, c) for a in S for b in S for c in S if a+b+c==0 and (a!=b or b!=c)]
...
first_of_tuples_list = [ (a, b, c) for a in S for b in S for c in S if a+b+c==0 and (a!=b or b!=c)][0]
...
odd_num_list_range = [n for n in range(0,100,2)]
#odd_num_list_range = [x for x in range(100)][::2]
...
value_list_modified_1 = [D[k] for D in dlist if k in D]# <-- Use the same expression here
print(value_list_modified_1)
k = 'Frodo'
value_list_modified_2 = [D[k] if k in D else "NOT_PRESENT" for D in dlist]  # <-- as you do here
...
listdict2dict = { name:salary for name,salary in zip(names,id2salary.values()) }
print(listdict2dict)

#def twice(farg): return 2*farg
#
#print(twice(5))
#print(twice('farg'))
#print(twice(['a',1]))
...
def nextInts(L): return [aNum +1 for aNum in L ]
print(nextInts([1,5,7]))


## 27: (Task 27) Procedure cubes
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def cubes(L): return [aNum**3 for aNum in L ] 
print(cubes([1,2,3]))
...
def all3digit(base, digits): return set([x*base**2 + y*base +z for x in digits for y in digits for z in digits])
print(all3digit(2, {0,1}))
print(all3digit(3, {0,1,2}))
